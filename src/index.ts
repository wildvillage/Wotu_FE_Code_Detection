...
async function runStep(): Promise<void> {
  ...
  step.info(`检测分支: ${params.sourceBranch === '*' || !params.sourceBranch ? '当前分支' : params.sourceBranch} ${sourceBranch ? `(${sourceBranch})` : '(HEAD)'}`);
  step.info(`目标分支: ${targetBranch}`);
  step.info(`ESLint检测模式: ${fullCheckMode ? '全量检测' : '增量检测'}`);
  step.info(`TypeScript检测模式: ${fullTypeScriptCheckMode ? '全量检测' : '增量检测'}`);
  ...
  if (enableESLintCheck) {
    ...
    try {
      ...
      if (result.success) {
        ...
        checkResults.push(`ESLint: 通过 (${result.checkedFiles.length} 个文件, ${result.warningCount} 个警告)`);
      } else {
        step.error(
        ...
        checkResults.push(`ESLint: 失败 (${result.errorCount} 个错误, ${result.warningCount} 个警告)`);
      }
    ...
    try {
      ...
      if (result.success) {
        if (result.checkedFiles === 0) {
          ...
          const fileCountMsg = result.checkedFiles === -1 ? '全量检查' : `检查了 ${result.checkedFiles} 个文件`;
          step.success(`TypeScript 检查通过! ${fileCountMsg}`);
          if (result.warningCount > 0) {
            step.warning(`发现 ${result.warningCount} 个警告`);
          }
          checkResults.push(`TypeScript: 通过 (${fileCountMsg}, ${result.warningCount} 个警告)`);
        }
      } else {
        const fileCountMsg = result.checkedFiles === -1 ? '全量检查' : `检查了 ${result.checkedFiles} 个文件`;
        step.error(
          `TypeScript 检查失败! 发现 ${result.errorCount} 个错误${result.warningCount > 0 ? `, ${result.warningCount} 个警告` : ''} (${fileCountMsg})`
        );
        hasErrors = true;
        totalErrors += result.errorCount;
        checkResults.push(`TypeScript: 失败 (${result.errorCount} 个错误, ${result.warningCount} 个警告, ${fileCountMsg})`);
      }
      totalWarnings += result.warningCount;
    } catch (error: any) {
  ...
  if (hasErrors) {
    step.error(`代码检查失败! 总计发现 ${totalErrors} 个错误, ${totalWarnings} 个警告`);
    process.exit(1);
  } else {
    step.success(`代码检查通过! ${totalWarnings > 0 ? `发现 ${totalWarnings} 个警告` : '没有发现问题'}`);
  }
}
